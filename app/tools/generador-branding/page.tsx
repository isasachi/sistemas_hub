...
export default function GeneradorBranding() {
  ...
  const preview = (
    <PreviewPanel
      ...
      tips={[{ text: <><strong className="text-[#f59e0b]">Tip:</strong> Un branding consistente genera hasta 3× más reconocimiento de marca en 6 meses.</> }]}
    />
  );
      ...
      {({ currentStep }) => (
        <div>
          {currentStep === 0 && (
            <div className="flex flex-col gap-5">
              <div>
                ...
                <p className="text-[14px] text-[#94a3b8]">Cuéntanos sobre tu empresa para generar una identidad única.</p>
              </div>
          ...
          {currentStep === 1 && (
            <div className="flex flex-col gap-5">
              <div>
                <h2 className="text-[22px] font-extrabold text-[#f1f5f9] tracking-[-0.3px] mb-1">Identidad de marca</h2>
                <p className="text-[14px] text-[#94a3b8]">Define los valores y el estilo visual que quieres proyectar.</p>
              </div>
              <div>
                <label className="text-[13px] font-semibold text-[#f1f5f9] mb-1 block">Valores de marca <span className="text-[#f59e0b]">*</span> <span className="text-[#475569] font-normal">(elige hasta 3)</span></label>
              ...
              <div>
                <label className="text-[13px] font-semibold text-[#f1f5f9] mb-1 block">Estilo visual <span className="text-[#f59e0b]">*</span></label>
          ...
          {currentStep === 3 && (
            <div className="flex flex-col gap-5">
              <div>
                <h2 className="text-[22px] font-extrabold text-[#f1f5f9] tracking-[-0.3px] mb-1">¡Tu branding está listo!</h2>
                <p className="text-[14px] text-[#94a3b8]">Aquí tienes los elementos clave de la identidad de tu marca.</p>
              </div>

              {/* Nombre y slogan */}
              <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-5">
                <p className="text-[11px] font-bold text-[#475569] tracking-[1.5px] uppercase mb-3">Nombre y Slogan</p>
                <p className="text-[22px] font-extrabold text-[#f1f5f9] mb-1">{MOCK_BRANDING.nombre}</p>
                <p className="text-[15px] text-[#94a3b8] italic">"{MOCK_BRANDING.slogan}"</p>
              </div>

              {/* Colores */}
              <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-5">
                <p className="text-[11px] font-bold text-[#475569] tracking-[1.5px] uppercase mb-3">Paleta de Colores</p>
                <div className="flex flex-col gap-2">
                  {MOCK_BRANDING.colores.map((c) => (
                    <div key={c.hex} className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-lg flex-shrink-0" style={{ background: c.hex }} />
                      <div>
                        <p className="text-[13px] font-semibold text-[#f1f5f9]">{c.name} — {c.label}</p>
                        <p className="text-[12px] text-[#475569] font-mono">{c.hex}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Voz */}
              <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-5">
                <p className="text-[11px] font-bold text-[#475569] tracking-[1.5px] uppercase mb-2">Voz de Marca</p>
                <p className="text-[14px] text-[#94a3b8] leading-[1.6]">{MOCK_BRANDING.voz}</p>
              </div>
            </div>
          )}
        </div>
