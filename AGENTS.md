...
⚠️ **El detalle forense no siempre cabe en 4096 chars** (con ~11 beats, el análisis completo llega a ~6300). `buildVideoPrompt` (parte del PLAN B) armará por niveles y mandará el más detallado que entre: sueltando primero `visual` (que se solapa con `action`) y `camera` al final, porque es corto y es lo que sostiene el encuadre. Sin esto el guard de `KIE_PROMPT_MAX` bloqueará el render.
