...
**El índice de paso es una constante compartida (`lib/video-ads/steps.ts`, `STEP`), no un número repetido.** Las cuatro rutas que escriben `video_sessions.step`, `VideoWizard.tsx` y cada sección del wizard importan `STEP` en vez de escribir el índice a mano. Nació de un bug real: el recableado corrió "Producto" de índice 2 a 1 y "Personaje" de 3 a 2, y `analyze-product/route.ts` se quedó escribiendo `step: 3` (el índice viejo). Al reanudar una sesión justo después de ese paso, el wizard aterrizaba en "Validación" con `validation` en null, y esa sección hace `if (!validation) return null` — pantalla en blanco, y de paso se saltaba el paso que recoge etnia y acento.
...
2. **El análisis forense no pasa por `callStructured`** (es OpenAI-primario y gpt-4o-mini no come video): usa `geminiCallStructured` directo vía `analyze-reference`. **El render de video es trabajo del PLAN B** — no existe hoy en esta rama.

**Contrato con KIE (`lib/video-ads/kie.ts`, verificado en `docs.kie.ai/market/grok-imagine/1-5-preview`).** El modelo es **`grok-imagine-video-1-5-preview`** (duración INTEGER 1–15, prompt máx 4096, `input` con `additionalProperties:false` rechaza `mode`). ⚠️ Existe un segundo modelo en el marketplace, `grok-imagine/image-to-video`, con **schema incompatible** (duración STRING 6–30, SÍ acepta `mode`, prompt 5000). Se descartó porque el hub necesita 30s de video y este topa en 15s. **Cambiar `MODEL` obliga a revisar el bloque entero de reglas, no solo el string** — `clampDuration`, `KIE_PROMPT_MAX`, los ranges del wizard, todo depende del modelo. `POST /api/v1/jobs/createTask` → `taskId`; `GET /api/v1/jobs/recordInfo?taskId=` → `data.state` (`waiting|queuing|generating|success|fail`) y el video en `JSON.parse(data.resultJson).resultUrls[0]`. Reglas que rompen el render en silencio si se ignoran, todas blindadas por `kie.test.ts`: `duration` es **entero** 1–15 (`clampDuration` redondea al rango); `aspect_ratio` default es `auto` (hay que forzar 9:16) **pero solo aplica en modo multi-imagen** — con una sola imagen hereda el ratio del origen; **`1080p` solo admite una imagen** → el hub renderiza **720p fijo**; `prompt` topa en **4096** caracteres; las imágenes se referencian como `@image(1)`, `@image(2)`… en el orden del array.

⚠️ **Con UNA imagen, el 9:16 del body es inerte — decisión de diseño para PLAN B.** Grok ignora `aspect_ratio` cuando el array trae una sola imagen y hereda el ratio del origen. La solución (enlienzar a 1080x1920 vía `vertical.ts` antes de crear la tarea) es padding, no recorte: un `cover` le cortaría los bordes al producto y contradiría la regla que KIE espera ("visually identical"). La lógica condicional será `images.length === 1`, no un `mode` inexistente.

⚠️ **El rango 1–15s es del render, no del guión — PLAN B lo recuperará.** El clamp en `kie.ts` es estricto porque si un guión producido en Task 2 saliera fuera de rango, el render fallaría minutos después con la cuota ya gastada. Un cambio de modelo mueve el rango en un solo lugar.
...
1. **Elementos gráficos separados.** Los subtítulos, watermarks, gráficos y overlays están en el campo `elementosGraficos` específico (`forensic.ts:91–93`), no en `accion` ni `camara`. Así el motor de render (PLAN B) entiende cuáles son artefactos del original y cuáles son estructura: sabe qué NO reproducir.

2. **Sin inferencia de casting.** El forense captura `sujeto` descriptivamente (edad, tono de piel, cabello, complexión) pero `buildForensicInstruction` prohíbe inferir etnia u origen cultural desde la apariencia (`forensic.ts:116–118`). El casting es una decisión del usuario en FASE 0, no una lectura del video.
...
4. **Duración del guión, no de la referencia.** La duración total sale del análisis (`ForensicReport.duracionTotalSeg`), pero el guión generado en PHASE 2 puede tener una duración distinta porque el cierre de plataforma (que el original tiene) no aparece en el copy rellenado. **El render final (PLAN B) la recalculará sobre el guión confirmado** — si las marcas de tiempo del análisis no son confiables, usará los valores de la BD.

⚠️ **El detalle forense no siempre cabe en 4096 chars** (con ~11 beats, el análisis completo llega a ~6300). PLAN B tendrá que armar un prompt que priorice lo esencial: suelta primero datos que se solapan (como `visual` y `accion`) y reserva al final los detalles de cámara, que son cortos pero sostienen el encuadre. Sin esta estrategia, el guard de `KIE_PROMPT_MAX` (4096) bloqueará el render antes de ni intentarlo.

**Plantilla extraída (PHASE 2, `Section4Template`).** La sección muestra el esqueleto extraído de la referencia y la template Fill in the Blank. **El editor de copy y el render son del PLAN B** — esta rama solo persiste la template en la DB (`supabase/migrations/20260812000001_video_spec_rewire.sql`, columna `template`). El tope de prompt de KIE (`KIE_PROMPT_MAX`, 4096) será crítico en PLAN B antes de crear la tarea — pasarse costaría un 422 con la cuota ya consumida.

**Costo — hoy y PLAN B.** El render (PLAN B) será la llamada más cara del hub por un orden de magnitud: kind `video-render` con tope **1 gen + 2 regens** (`GEN_VIDEO_LIMIT`), aparte del backstop diario global. Ese cap sigue reservado para cuando el render exista — no se borró al eliminar el render de esta rama.

Sin render, la llamada más cara que SÍ corre hoy es el análisis forense (`video-forensic`, `analyze-reference`): manda hasta 14 MB de video a Gemini, no es gratis. Antes de este fix se quedó sin tope per-step (no estaba en `IMAGE_KINDS`, así que solo lo cubría el backstop global de 500/día) — el cap se movió del paso que se eliminó (render) al paso caro que quedó vivo. Ahora `video-forensic` está en `IMAGE_KINDS` con el cap genérico (1 gen + 3 regens): es texto, no imagen, pero reutiliza el mismo mecanismo por ser igual de caro; no tiene su propio branch en `limitFor` porque no lo necesita. `video-template` (extraer la plantilla, "Extraer otra vez" en `Section4Template`) sí sigue sin tope per-step a propósito: reprocesa el análisis forense YA guardado, no vuelve a mandarle video a Gemini.

**Gate de assets verticales (bloqueante, `upload-client.ts` + `Section0Reference`).** El video de referencia se mide en el browser antes de subir y el wizard **no deja continuar** si es apaisado — el output es 9:16 y una fuente horizontal lo arruina río abajo. El criterio es `alto > ancho`, **no** 9:16 exacto: si la medición falla (HEIC, códec raro) **pasa igual** — dejar al usuario encerrado por un formato que el browser no decodifica es peor que un video horizontal. **La generación de personaje es PLAN B** — hoy la FASE 0 solo acepta inputs de texto (etnia, acento, descripción). El product image NO se valida por ratio: el PLAN B lo enlienza server-side si es necesario.
...
**Schema:** `supabase/migrations/20260810000001_video_sessions.sql` (base de video_sessions) + `20260812000001_video_spec_rewire.sql` (columnas de INPUTS y VALIDATION).
