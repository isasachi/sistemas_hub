...
**Costo — PLAN B.** El render será la llamada más cara del hub por un orden de magnitud: kind `video-render` con tope **1 gen + 2 regens** (`GEN_VIDEO_LIMIT`), aparte del backstop diario global (hoy aún sin consumidor en vivo). El guión seguirá sin tope per-step (es texto, y `isImageKind` no lo cubre): reescribirlo cuesta una fracción de un render. El análisis forense (PHASE 1) es cero-costo: Gemini devuelve estructuras, no tokens al usuario.
