...
5. **Cold start on-demand.** Un nicho nuevo se registra como `pending` (`search` hace `upsertNiche`, NO scrapea — Vercel no corre Playwright). El daemon siempre-on lo levanta en una vuelta del loop (minutos): `getNichesToRefresh()` devuelve los `pending`. Sin dispatch externo. Los scripts iteran nichos desde el DB (`getActiveNiches`/`getNichesToRefresh`), no desde el mapa estático.
   ⚠️ **La UI ya no puede disparar un cold start.** La tool no tiene barra de búsqueda: se navega solo con los chips de `top-niches` (los 36 nichos con más inventario) + un chip "Todos" que vuelve a los top picks. Los chips solo ofrecen nichos que YA tienen productos, así que `upsertRawNiche(niche, 'pending')` quedó inalcanzable desde el navegador — la lógica sigue en `search/route.ts` y responde a cualquier POST, pero para sembrar nichos nuevos hoy la vía real es `niches.txt` + `seed-niches.ts` en el worker. Si se quiere recuperar el cold start desde la web hay que devolverle a la UI alguna entrada de texto libre.
