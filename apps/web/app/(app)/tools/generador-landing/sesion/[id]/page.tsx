        ...
        {s && (
          <>
            <div className="flex items-start justify-between gap-4 mb-6">
              <div>
                <h1 className="text-[22px] font-bold text-[#efe7e0]">{s.product_name || "Landing sin nombre"}</h1>
                ...
                {sections.map((sec, i) => (
                  <div key={i} className="rounded-xl overflow-hidden bg-white/[0.03] border border-white/[0.06]">
                    {sec.imageUrl ? (
                      ...
                      <div className="p-6 text-[13px] text-[#a98c88]">{SECTION_LABELS[sec.type]} — sin imagen aún</div>
                    )}
                    <p className="text-[11px] text-[#a98c88] px-3 py-2">{SECTION_LABELS[sec.type]}</p>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-[13px] text-[#a98c88]">Esta sesión todavía no tiene secciones generadas.</p>
            )}
          </>
        )}
      </div>
