  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#14050a]">
      <div className="px-6 py-4">
        <Link href={STEPS[4].path} className="inline-flex items-center gap-2 h-9 px-3 rounded-xl border border-white/[0.1] text-[13px] font-semibold text-[#efe7e0] no-underline hover:bg-white/[0.05] transition-colors">
      ...
      <div className="flex-1 w-full max-w-[560px] mx-auto px-6 pb-10 flex flex-col gap-6">
        <div className="flex flex-col gap-2">
          <h1 className="text-[24px] font-bold text-[#efe7e0] leading-tight">
            Creando la marca de {brief.brandName}
          </h1>
          <p className="text-[13px] text-[#c9b4ae]">Toma alrededor de un minuto por pieza.</p>
        </div>
          ...
          {STEPS_UI.map((s, i) => {
            ...
            return (
              <div key={s} className={`flex items-center gap-3 rounded-xl border px-4 py-3 transition-colors ${
                st === 'running' ? 'border-[rgba(232,70,122,0.4)] bg-[rgba(232,70,122,0.06)]'
                : st === 'done' ? 'border-[rgba(44,207,111,0.25)]'
                : st === 'failed' ? 'border-red-500/30' : 'border-white/[0.06]'
              }`}>
                <span className="w-[22px] h-[22px] rounded-full border border-white/[0.12] bg-white/[0.03] flex items-center justify-center readout text-[11px] font-bold text-[#a98c88]">
                  {st === 'done' ? <Check className="w-3 h-3 text-[#3ed88a]" strokeWidth={3} />
                    : st === 'failed' ? <X className="w-3 h-3 text-red-400" strokeWidth={3} />
                    : st === 'running' ? <span className="w-3 h-3 border-2 border-white/20 border-t-[#e8467a] rounded-full animate-spin" />
                    : i + 1}
                </span>
                <span className="text-[13px] text-[#efe7e0] flex-1">{LABELS[s]}</span>
                <span className="text-[11px] text-[#a98c88]">
        ...
        <p className="text-[12px] text-[#a98c88] leading-relaxed">{TIPS[tip]}</p>

        {slow && sessionId && (
          <button type="button"
                  onClick={() => router.push(`/tools/generador-branding/nuevo/resultado?s=${sessionId}`)}
                  className="h-11 px-5 self-start rounded-xl border border-white/[0.14] text-[13px] font-semibold text-[#efe7e0] hover:bg-white/[0.05] transition-colors cursor-pointer bg-transparent">
