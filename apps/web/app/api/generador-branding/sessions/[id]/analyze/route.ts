...
import { extractTemplateExtras, buildPalettes } from '@/lib/branding/palette-variants'
...
export async function POST(
  ...
  // Las 3 paletas también en modo upload. Best-effort: si la segunda llamada
  // falla, la sesión sigue con la paleta de la imagen y una sola opción — no
  // vale abortar un análisis que ya salió bien por un extra.
  let paletteOptions = [extracted.palette]
  let containerType: string | null = null
  try {
    const extras = await extractTemplateExtras(data, fetchedMime, extracted.palette)
    paletteOptions = buildPalettes(extracted.palette, extras.variants)
    containerType = extras.containerType
  } catch (e) {
    console.error('[analyze] paletas alternativas', e)
  }
  ...
  await updateBrandingSession(id, {
    ...
    ...(containerType && !session.container_type ? { container_type: containerType } : {}),
  })
