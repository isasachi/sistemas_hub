    ...
    const parts: Part[] = [
      { inlineData: { mimeType, data: bytes.toString('base64') } },
      {
        text: [
          ...
          'Analyze the product image for a UGC video ad. The two description fields answer',
          'DIFFERENT questions and must not be merged:',
          '',
          'productDescription — the physical OBJECT: shape, size in the hand, material,',
          'closure, colors. Describe the object, never the photograph: no surface it rests',
          'on, no shadow, no light direction, no camera angle, no background, and never',
          'whether it floats. In the video someone holds it in a room.',
          '',
          'brandingDescription — a LETTER-BY-LETTER TRANSCRIPTION of the text printed on',
          'the package: brand, product name, claims, ingredient list, dose, volume. Copy',
          'what is written, in the language of the package; do NOT translate it and do NOT',
          'describe the graphic style. "Minimalist, clinical, high-end dermatological look"',
          'is a style description and is worthless here.',
          'This field is the ONLY source the ad script has for what the product contains:',
          'if an ingredient is not transcribed here, the script cannot name it, and a model',
          'asked for one anyway will supply it from memory and get it wrong.',
          '',
          'Return ProductScan JSON.',
        ].join('\n'),
      },
    ]
