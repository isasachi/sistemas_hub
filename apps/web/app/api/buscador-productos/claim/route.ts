...
// Reclamos del flujo de un producto por vez (motor de descubrimiento).
//
//   POST { accion: 'tomar',  dedupeKey, seed }        → lo saca del catálogo
//   POST { accion: 'cerrar', dedupeKey, encuesta, comodin }
//   GET                                               → cupo restante
//
// ⚠️ EL CUPO LO HACE CUMPLIR EL SERVIDOR, no la pantalla. El contador de la UI
// es informativo; lo que impide tomar el producto 6 con el plan 1 es este
// archivo. Un límite pintado en el cliente no es un límite — es la misma lección
// que el candado por rango del buscador.
//
...
export async function GET() {
  ...
  const [e, lista] = await Promise.all([
    ...
    claimsDe(id.userId).then((rows) => rows.map(toDiscoveryEntry)),
  ])
  return NextResponse.json({ ...e, lista })
}
...
export async function POST(req: NextRequest) {
  ...
  let body: { accion?: string; dedupeKey?: string; seed?: string; comodin?: boolean; encuesta?: Encuesta }
  try { body = await req.json() } catch {
    return NextResponse.json({ error: 'Petición inválida' }, { status: 400 })
  }
  const dedupeKey = typeof body.dedupeKey === 'string' ? body.dedupeKey : ''
  if (!dedupeKey) return NextResponse.json({ error: 'Falta el producto' }, { status: 400 })
  ...
  if (body.accion === 'tomar') {
    ...
    const tomado = await tomarProducto(id.userId, dedupeKey, body.seed ?? null)
    if (!tomado) {
