...
export default function Section2Product() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <p className="text-[11px] font-bold uppercase tracking-widest text-[#8a8a8a] mb-2">Imagen del producto *</p>
        <FileUpload label="Subir producto" onFile={(f) => { setProductFile(f); setProductPreview(URL.createObjectURL(f)) }} preview={productPreview} />
      </div>
      <div>
        <p className="text-[11px] font-bold uppercase tracking-widest text-[#8a8a8a] mb-2">Logo (opcional)</p>
      ...
      <div className="flex flex-col gap-3 pt-1">
        <p className="text-[12px] text-[#bdbdbd]">Tres preguntas rápidas:</p>
        {[
          ...
          <div key={key} className="flex items-center gap-2">
            <span className="w-5 h-5 rounded-full bg-white/[0.05] text-[#8a8a8a] text-[10px] font-bold flex items-center justify-center shrink-0">{i + 1}</span>
            <input
              type="text"
