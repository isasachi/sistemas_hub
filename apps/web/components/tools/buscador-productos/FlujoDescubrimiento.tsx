...
//
// Lo renderiza `tools/buscador-productos` cuando el chip está en
// "Descubrimiento". La lista clásica sigue siendo la otra vista.
//
// ⚠️ SIRVE EL MISMO INVENTARIO QUE LA LISTA (`ph_*`). Del prototipo `disc_*` se
// portó ESTA PANTALLA y nada más: el worker de aquel motor no se trajo, así que
// lo que alimenta el flujo es el scraper de producción. Por eso las "semillas"
// son las categorías que ya existen y no las de aquel pipeline.
//
...
import { CATEGORIES, type RawProductEntry, type RawSearchResponse } from "@ph/shared";
import {
...
// Las semillas llegan como ids de categoría (`@ph/shared`); el chip muestra su
// nombre. Un id sin etiqueta se pinta crudo en vez de desaparecer: prefiero un
// chip feo a una categoría que el usuario no puede elegir.
const ETIQUETA = new Map<string, string>(CATEGORIES.map((c) => [c.id, c.label]));
const etiquetaDe = (id: string) => ETIQUETA.get(id) ?? id;
...
export default function FlujoDescubrimiento() {
  ...
  const traer = useCallback(async (seed: string | null) => {
    const res = await fetch("/api/buscador-productos/search", {
      ...
      body: JSON.stringify({ motor: "flujo", seed: seed ?? undefined }),
    });
  ...
  const abrir = useCallback(async () => {
    ...
    const res = await fetch("/api/buscador-productos/claim", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ accion: "tomar", entryId: actual.id, seed: nicho }),
    });
  ...
  const cerrar = useCallback(async (comodin: boolean) => {
    ...
    const res = await fetch("/api/buscador-productos/claim", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ accion: "cerrar", entryId: actual.id, encuesta, comodin }),
    });
    if (!res.ok) {
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {!semillas.length && (
              <p className="text-[13px] text-[#a98c88]">
                Todavía no hay categorías con productos disponibles. Vuelve en un rato.
              </p>
            )}
          </div>
